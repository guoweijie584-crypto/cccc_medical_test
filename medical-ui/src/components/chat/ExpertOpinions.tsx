...
const EXPERT_CONFIG: Record<string, { icon: string; label: string; color: string; bg: string }> = {
  pharmacist:   { icon: '💊', label: '药剂师',     color: 'text-blue-400',   bg: 'bg-blue-500/10 border-blue-500/20' },
  nutritionist: { icon: '🥗', label: '营养师',     color: 'text-green-400',  bg: 'bg-green-500/10 border-green-500/20' },
  doctor:       { icon: '🏥', label: '代谢病医生', color: 'text-purple-400', bg: 'bg-purple-500/10 border-purple-500/20' },
};
...
function getExpertConfig(key: string) {
  ...
  return { icon: '👨‍⚕️', label: key, color: 'text-gray-400', bg: 'bg-white/5 border-white/10' };
}
  ...
  return (
    <div className="mt-3 border-t border-white/10 pt-2">
      ...
      >
        ...
        <span className="text-gray-600 group-hover:text-gray-500">({entries.length}位专家)</span>
      </button>
              ...
              {entries.map(([expert, opinion]) => {
                ...
                return (
                  <motion.div
                    ...
                    transition={{ duration: 0.2 }}
                    className={`rounded-lg border px-3 py-2.5 ${config.bg}`}
                  >
