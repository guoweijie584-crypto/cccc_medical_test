...
export async function sendMessage(
  ...
  if (files && files.length > 0) {
    ...
    if (refs.length > 0) form.append("refs_json", JSON.stringify(refs));
  ...
  return apiJson(`/api/v1/groups/${encodeURIComponent(groupId)}/send`, {
    method: "POST",
    body: JSON.stringify({ text, by: "user", to, path: "", priority, reply_required: replyRequired, client_id: clientId, refs }),
  });
}
...
export async function replyMessage(
  ...
  return apiJson(`/api/v1/groups/${encodeURIComponent(groupId)}/reply`, {
    method: "POST",
    body: JSON.stringify({ text, by: "user", to, reply_to: replyTo, priority, reply_required: replyRequired, client_id: clientId, refs }),
  });
}
