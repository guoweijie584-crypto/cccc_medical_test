...
import { buildPatientContextRefs, sanitizePatientMetadataPreview } from "../utils/chatMetadata";
...
export function useChatTab({
  ...
  const selectedPatientBinding = useMedicalStore(
    useCallback((state) => state.bindingsByGroup[String(selectedGroupId || "").trim()] || null, [selectedGroupId])
  );
  ...
  const sendMessage = useCallback(async () => {
    ...
    const shouldInjectPatientBinding =
      !!selectedPatientBinding &&
      !isCrossGroup &&
      groupDoc?.title === "glucose-management-main" &&
      (
        toTokens.includes("primary") ||
        toTokens.includes("@foreman") ||
        (toTokens.length === 0 && (groupSettings?.default_send_to || "foreman") === "foreman")
      );
    const patientContextRefs = shouldInjectPatientBinding ? buildPatientContextRefs(selectedPatientBinding) : [];
    const outboundText = txt;
    ...
    try {
      ...
      if (replyTargetSnapshot) {
        ...
        if (isCrossGroup) {
          resp = await api.sendCrossGroupMessage(selectedGroupId, dstGroup, outboundText, to, prio, replyRequiredSnapshot);
        } else {
