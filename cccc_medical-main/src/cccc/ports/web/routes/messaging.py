...
def create_routers(ctx: RouteContext) -> list[APIRouter]:
    ...
    def _parse_refs_json(raw: str) -> list[dict[str, Any]]:
        try:
            ...
            raise HTTPException(status_code=400, detail={"code": "invalid_refs", "message": f"invalid refs_json: {exc}"}) from exc
        if not isinstance(parsed, list):
            raise HTTPException(status_code=400, detail={"code": "invalid_refs", "message": "refs_json must decode to a list"})
        refs: list[dict[str, Any]] = []
        for item in parsed:
            if not isinstance(item, dict):
                raise HTTPException(status_code=400, detail={"code": "invalid_refs", "message": "each ref must be an object"})
