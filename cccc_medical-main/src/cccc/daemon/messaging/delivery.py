...
FAST_RETRY_FLUSH_SECONDS = 1.0  # Short retry when a PTY submit fails after queueing
### NOTE
...
def _schedule_retry_flush(group: Group, *, actor_id: str, delay_s: float = FAST_RETRY_FLUSH_SECONDS) -> None:
    gid = group.group_id
    aid = actor_id
    ...
    def delayed_retry() -> None:
        try:
            ...
            logger.exception("[flush] %s/%s fast retry flush failed", gid, aid)
    ...
    if preamble_just_sent and message_text:
        ...
        def delayed_message_send():
            ...
            if ok:
                ...
                THROTTLE.requeue_front(gid, aid, messages)
                _schedule_retry_flush(group, actor_id=aid)
