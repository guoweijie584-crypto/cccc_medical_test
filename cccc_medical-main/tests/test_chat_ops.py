...
class TestChatOps(unittest.TestCase):
    ...
    def test_reply_to_pty_recipient_flushes_pending_messages_immediately(self) -> None:
        _, cleanup = self._with_home()
        try:
            create, _ = self._call("group_create", {"title": "chat-reply-flush", "topic": "", "by": "user"})
            self.assertTrue(create.ok, getattr(create, "error", None))
            group_id = str((create.result or {}).get("group_id") or "").strip()
            self.assertTrue(group_id)

            for actor_id in ("primary", "memory"):
                add, _ = self._call(
                    "actor_add",
                    {
                        "group_id": group_id,
            ...
            with (
                patch("cccc.daemon.messaging.chat_ops.queue_chat_message") as queue_mock,
                patch("cccc.daemon.messaging.chat_ops.flush_pending_messages", return_value=True) as flush_mock,
            ):
                reply, _ = self._call(
                    "reply",
                    {
            ...
            self.assertEqual(flush_mock.call_args.kwargs.get("actor_id"), "memory")
        finally:
            cleanup()

    def test_send_preserves_refs_and_only_renders_medical_context_in_delivery_text(self) -> None:
        _, cleanup = self._with_home()
        try:
            create, _ = self._call("group_create", {"title": "chat-refs", "topic": "", "by": "user"})
            self.assertTrue(create.ok, getattr(create, "error", None))
            group_id = str((create.result or {}).get("group_id") or "").strip()
            self.assertTrue(group_id)
            ...
            refs = [
                {
                    ...
                    "text": "[PATIENT_BINDING patient_id=PAT001 patient_name=张三]\n[PATIENT_PROFILE]\nname=张三\nage=45\n[/PATIENT_PROFILE]",
                    "medical_context": {
                        "patient_id": "PAT001",
                        "patient_name": "张三",
                        "profile": {"name": "张三", "age": 45},
                    },
                }
            ]

            with patch("cccc.daemon.messaging.chat_ops.queue_chat_message") as queue_mock:
                send, _ = self._call(
                    "send",
                    {
                        "group_id": group_id,
            ...
            self.assertEqual(str(data.get("text") or ""), "请帮我看一下最近的血糖趋势")
            stored_refs = data.get("refs") if isinstance(data.get("refs"), list) else []
            self.assertEqual(len(stored_refs), 1)
            self.assertEqual(str(stored_refs[0].get("title") or ""), "medical_context")
            self.assertEqual(str(stored_refs[0].get("text") or ""), refs[0]["text"])
            self.assertEqual(stored_refs[0].get("medical_context"), refs[0]["medical_context"])

            queue_mock.assert_called_once()
            delivered_text = str(queue_mock.call_args.kwargs.get("text") or "")
            self.assertIn("[PATIENT_BINDING patient_id=PAT001 patient_name=张三]", delivered_text)
            self.assertIn("[PATIENT_PROFILE]", delivered_text)
            self.assertIn("请帮我看一下最近的血糖趋势", delivered_text)
        finally:
            cleanup()

    def test_send_to_pty_recipient_flushes_pending_messages_immediately(self) -> None:
        _, cleanup = self._with_home()
        try:
            create, _ = self._call("group_create", {"title": "chat-send-flush", "topic": "", "by": "user"})
            self.assertTrue(create.ok, getattr(create, "error", None))
            group_id = str((create.result or {}).get("group_id") or "").strip()
            self.assertTrue(group_id)
    ...
    def test_reply_required_send_to_pty_recipient_emits_immediate_system_notify(self) -> None:
        _, cleanup = self._with_home()
        try:
            create, _ = self._call("group_create", {"title": "chat-send-notify", "topic": "", "by": "user"})
            self.assertTrue(create.ok, getattr(create, "error", None))
            group_id = str((create.result or {}).get("group_id") or "").strip()
            self.assertTrue(group_id)
            ...
            with (
                patch("cccc.daemon.messaging.chat_ops.queue_chat_message") as queue_mock,
                patch("cccc.daemon.messaging.chat_ops.flush_pending_messages", return_value=True) as flush_mock,
                patch("cccc.daemon.messaging.chat_ops.emit_system_notify") as notify_mock,
            ):
                send, _ = self._call(
                    "send",
                    {
            ...
            self.assertEqual(notify_kwargs.get("by"), "system")
            notify = notify_kwargs.get("notify")
            self.assertIsNotNone(notify)
            self.assertEqual(getattr(notify, "target_actor_id", ""), "memory")
        finally:
            cleanup()
